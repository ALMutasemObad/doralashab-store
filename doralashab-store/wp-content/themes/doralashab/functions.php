...
/**
 * One-time rollback cleanup for the temporary National Day school campaign.
 * Products and their generated campaign images are moved to Trash so the
 * operation remains recoverable until the cleanup code is removed.
 */
function doralashab_rollback_national_day_products(): void {
	...
	$campaign_skus = array(
		...
		'ND-2026-COLORS',
		'ND-2026-KIT',
		'ND-2026-BUNDLE-STUDENT',
		'ND-2026-BUNDLE-SIBLINGS',
	);

	foreach ( $campaign_skus as $sku ) {
		$product_id = wc_get_product_id_by_sku( $sku );
		if ( ! $product_id || 'trash' === get_post_status( $product_id ) ) {
			continue;
		}
		...
		if ( $image_id && $image_path && 0 === strpos( basename( $image_path ), 'national-day-' ) ) {
			wp_trash_post( $image_id );
		}
	}
...
add_action( 'init', 'doralashab_rollback_national_day_products', 30 );
